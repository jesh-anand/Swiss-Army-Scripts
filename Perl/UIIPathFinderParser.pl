...
sub checkUniqueIdentifier {
	...
	if ($day > 1) {
		my $errorCode 	= 'E005';
		my $errorMsg	= " ERROR [" . $func . "] : " .$errorCode. " : No UII Feed has been processed for the past " . int($day) . " day(s)! 
		Process has been halted until previous day(s) UII Feed has been received.";		
		&sendErrorEmail( $func, $file , $errorCode, $errorMsg );						
		&writeLog($errorMsg);
