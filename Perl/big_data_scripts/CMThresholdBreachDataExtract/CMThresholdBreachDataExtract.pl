...
sub getProcessingDate(){
	...
	if(scalar(@arg)==2){
		...
		if ($lastProcessingDate) {
			...
			$arg[0] = &getCurrentProcessingDate(getProcessingDatePlusOne($lastProcessingDate));
...
#======================================================================================
# Name 		    : getProcessingDatePlusOne
# Description	: Generate the daily processing date plus one if last processed time is found
# Input		    : DateTime
# Output	    : DateTime
# Author	    : Harish Kumar
# Global Var	: -
# Date		    : 22 Mar 2016
#======================================================================================
sub getProcessingDatePlusOne(){
	my ($dateTime) 	= @_;
	my $func 		= "getProcessingDatePlusOne";
	my @startDateArry 	= split(/-/,$dateTime);
	my $epochDateTime 	= timelocal("00","00","00",$startDateArry[0],$startDateArry[1]-1,$startDateArry[2]);

	my $newEpochDateTime = $epochDateTime  + 86400;
	return $newEpochDateTime ;
}
...
sub transferFile
{
	...
	foreach my $file (@allFiles){
		...
		my $curlCmd = "bash ".LIB_PATH."/httpfs-file-transfer.sh ".$file." ".$finalDestPath."/";
