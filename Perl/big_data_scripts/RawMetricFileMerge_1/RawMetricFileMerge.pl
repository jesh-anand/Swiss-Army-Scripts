...
# Author		: 607980248 , 607133064, 608750727
# Date			: 02/11/2015
# Version		: 0.0.1
...
sub processTrapMetricFileMerge(){
	...
	my($trapDir,$curHour) = getTrapDetails($eachSubProfileDir);
	...
	my($year1,$month1,$day1,$hour1) = getMetricHourlyDirTimestamp($curHour);
...
sub processRawMetricFileMerge(){
	...
	my $csvFileCount = 0;
	my($year1,$month1,$day1,$hour1) = getPreviousMetricHourlyDirTimestamp($metricHourlyDirPath);
...
sub getPreviousMetricHourlyDirTimestamp(){
	my($dirtmp)=@_;
	my $entryPathDirName = basename($dirtmp);
	$entryPathDirName =~/(\d{8,8}\_\d{2,2})/;
	my $year1 = substr $1, 4, 4;
